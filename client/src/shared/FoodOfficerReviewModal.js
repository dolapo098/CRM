...
import { TextField } from "./_formcomponents";
...
export function FoodOfficerReviewModal(props) {
  ...
  const reject = () => {
    if (status === appStateData.status.awaitngFoodProcessingOfficer) {
      setFormData({
        ...formData,
        state: appStateData.state.rejectedByFoodProcessingOfficer,
      });
    }
  };
  ...
  const onSubmit = ({ food_officer_comment }, { setStatus, setSubmitting }) => {
    setStatus();
    setIsSuccess(false);
    complaintsService
      .foodProcessingOfficerreview({ id, food_officer_comment, state })
      .then(
        (res) => {
          setIsSuccess(true);
          setSubmitting(false);
      ...
      <>
        <Modal show={props.show} onHide={handleClose}>
          <Modal.Header closeButton>
            <h6>Food Officer Review Task</h6>
          </Modal.Header>
          <Modal.Body>
